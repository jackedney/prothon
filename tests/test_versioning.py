...
class TestParseVersion:
    def test_parse_version_extracts_components(self):
    ...
    def test_parse_version_rejects_missing_patch(self):
        with pytest.raises(VersionError, match="invalid version"):
            parse_version("1.2")
    ...
    def test_parse_version_parametrized(self, v, expected):
...
class TestBumpMajor:
    ...
    def test_bump_major_with_v_prefix(self):
...
class TestUpdatePyprojectVersion:
    ...
    def test_update_pyproject_preserves_comments(self, tmp_path):
        pyproject = tmp_path / "pyproject.toml"
        pyproject.write_text('[project]\nversion = "0.1.0"\n# comment\n')
        update_pyproject_version(pyproject, "0.2.0")
        content = pyproject.read_text()
    ...
    def test_update_pyproject_preserves_other_tables(self, tmp_path):
        pyproject = tmp_path / "pyproject.toml"
    ...
    def test_update_pyproject_raises_on_missing_project_table(self, tmp_path):
        pyproject = tmp_path / "pyproject.toml"
        pyproject.write_text("[tool.ruff]\nline-length = 88\n")
        with pytest.raises(
            VersionError, match="pyproject.toml missing \\[project] table"
        ):
            update_pyproject_version(pyproject, "0.2.0")

    def test_update_pyproject_raises_on_invalid_toml(self, tmp_path):
        pyproject = tmp_path / "pyproject.toml"
...
class TestUpdateInitVersion:
    def test_update_init_version_updates_double_quotes(self, tmp_path):
        init = tmp_path / "__init__.py"
        init.write_text('__version__ = "0.1.0"\n')
        update_init_version(init, "0.2.0")
        assert '__version__ = "0.2.0"' in init.read_text()

    def test_update_init_version_updates_single_quotes(self, tmp_path):
        init = tmp_path / "__init__.py"
        init.write_text("__version__ = '0.1.0'\n")
        update_init_version(init, "0.2.0")
        assert '__version__ = "0.2.0"' in init.read_text()

    def test_update_init_version_handles_whitespace(self, tmp_path):
        init = tmp_path / "__init__.py"
        init.write_text('__version__  =  "0.1.0"\n')
        update_init_version(init, "0.2.0")
        assert '__version__ = "0.2.0"' in init.read_text()

    def test_update_init_version_preserves_other_content(self, tmp_path):
        init = tmp_path / "__init__.py"
        init.write_text(
            '"""Module docstring."""\n__version__ = "0.1.0"\n\ndef foo(): pass\n'
        )
    ...
    def test_update_init_version_raises_when_no_version(self, tmp_path):
        init = tmp_path / "__init__.py"
...
class TestCreateTag:
    def test_create_tag_creates_annotated_tag(self, tmp_path):
    ...
    def test_create_tag_uses_v_prefix(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "README.md").write_text("# test\n")
        run_git("add", ".", cwd=tmp_path)
        run_git("config", "user.email", "test@example.com", cwd=tmp_path)
...
class TestDetectBumpType:
    def test_detect_bump_type_returns_major_for_spec_change(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "docs").mkdir()
    ...
    def test_detect_bump_type_returns_minor_for_design_change(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "docs").mkdir()
    ...
    def test_detect_bump_type_returns_patch_for_patterns_change(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "docs").mkdir()
    ...
    def test_detect_bump_type_returns_patch_for_source_change(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "src").mkdir()
    ...
    def test_detect_bump_type_returns_none_for_readme_only(self, tmp_path):
        run_git("init", cwd=tmp_path)
    ...
    def test_detect_bump_type_prioritizes_spec_over_design(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "docs").mkdir()
        (tmp_path / "docs" / "SPEC.md").write_text("# spec\n")
        (tmp_path / "docs" / "DESIGN.md").write_text("# design\n")
    ...
    def test_detect_bump_type_prioritizes_design_over_patterns(self, tmp_path):
        run_git("init", cwd=tmp_path)
        (tmp_path / "docs").mkdir()
        (tmp_path / "docs" / "DESIGN.md").write_text("# design\n")
        (tmp_path / "docs" / "PATTERNS.md").write_text("# patterns\n")
