...
version-bump:
  ...
  script:
    - |
      ...
      subprocess.run(["git", "config", "credential.helper", "!f() { echo username=gitlab-ci-token; echo \"password=$GITLAB_TOKEN\"; }; f"], check=True)
      remote_url = f"https://{os.environ['CI_SERVER_HOST']}/{os.environ['CI_PROJECT_PATH']}.git"
