...
`GitDiffProvider` enables subprocess-free testing of promise verification. `AssistantBackend` enables pluggable assistant backends with a shared launch lifecycle.
...
`ProthonError` is the catch-all at the CLI boundary. Specific subclasses allow callers to handle individual failure modes when needed, but the CLI only needs to catch the base.
