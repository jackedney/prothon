...
Each command launches a separate assistant session. Do NOT invoke `/spec-writer`, `/design-writer`, or `/patterns-writer` directly — the user runs these via the CLI.
...
Project-specific reference skills live in `.agents/skills/` as directories containing `SKILL.md`. Claude Code, opencode, and Gemini CLI discover this directory natively — no symlinks or additional configuration needed.

When creating new skills, place them in `.agents/skills/<skill-name>/SKILL.md`.
