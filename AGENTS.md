...
Prothon built-in skills (prefixed `prothon-`) are bundled in the package at `src/prothon/skills/` and symlinked into `~/.claude/skills/` on every CLI invocation.

Project-specific reference skills (tech-*, style-*, etc.) generated by the tech-researcher live in each project's `.agents/skills/` directory. For prothon's own development, these are in `.agents/skills/`.
