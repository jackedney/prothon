...
Prothon built-in skills (prefixed `prothon-`) are bundled in the package at `src/prothon/skills/` and symlinked into the assistant's skill discovery directory when launching a skill (i.e., when `launch()` calls `backend.sync_skills()` from the spec, design, patterns, execute, or compliance commands).
