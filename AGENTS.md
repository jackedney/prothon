...
Prothon built-in skills (prefixed `prothon-`) are bundled in the package at `src/prothon/skills/` and symlinked into `~/.claude/skills/` on every CLI invocation. The `.agents/skills` symlink in this repo points to `src/prothon/skills/` for development convenience.

Project-specific reference skills (tech-*, style-*, etc.) generated by the tech-researcher live in each project's `.agents/skills/` directory.
